#!/usr/bin/env python3
"""
DOCX TO GOOGLE DOCS CONVERTER
Converts Word documents (.docx) to Google Docs in the same folder.
Keeps both the original .docx and creates a linked .gdoc file.
"""
...
def convert_docx_to_gdoc(service, docx_file, folder_id):
    """Upload .docx and convert to Google Doc"""
    file_metadata = {
        'name': docx_file.stem,  # filename without extension
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        safe_print("Usage: python CONVERT_DOCX_TO_GDOCS.py <folder_path>")
        safe_print("\nExample:")
        safe_print('  python CONVERT_DOCX_TO_GDOCS.py "G:\\My Drive\\00 - Trajanus USA\\01-Core-Protocols"')
        sys.exit(1)

    safe_print("=" * 70)
    safe_print("DOCX TO GOOGLE DOCS CONVERTER")
    safe_print("=" * 70)
    ...
    docx_files = [f for f in source_folder.glob('*.docx') if not f.name.startswith('~$')]
    safe_print(f"Found: {len(docx_files)} Word documents")

    if len(docx_files) == 0:
        safe_print("[WARNING] No .docx files found")
        sys.exit(0)
    ...
    for docx_file in sorted(docx_files):
        safe_print(f"[CONVERT] {docx_file.name}")
        ...
        # Upload and convert
        result = convert_docx_to_gdoc(service, docx_file, folder_id)
    ...
    safe_print("COMPLETE")
    safe_print("=" * 70)
    safe_print(f"Total: {len(docx_files)}")
